# WALK-FORWARD LINEAR BOX-COX AICC MODEL - FIXED VERSION
# Replaces the problematic lm_cov_bxcx_aicc_walkforward.R

update_fc_w_lm_cov_bxcx_aicc <- function(
    stock_name,
    raw_data_short,
    fc,
    forecast_yr,
  ...
  # Simplified approach: Just use Box-Cox transformation without complex AICc
  # (The AICc part was causing scoping issues)
  fit_boxcox_model <- function(train_data, valid_covars) {
    
    tryCatch({
      # Simple Box-Cox approach
      ...
      if (abs(lambda_optimal) < 1e-6) {
        temp_data$y_transformed <- log(y_vals)
      } else {
        temp_data$y_transformed <- (y_vals^lambda_optimal - 1) / lambda_optimal
      }
      
      # Fit final model
      final_formula <- as.formula(paste("y_transformed ~", paste(valid_covars, collapse = " + ")))
      final_model <- lm(final_formula, data = temp_data)
      
      # Store transformation info
      final_model$lambda <- lambda_optimal
      final_model$y_offset <- offset
      
      return(final_model)
      
    }, error = function(e) {
      cat("      Box-Cox + AICc failed, using linear model:", e$message, "\n")
      ...
      linear_model$lambda <- 1  # No transformation
      linear_model$y_offset <- 0
      return(linear_model)
    })
  }
  
  # Custom predict function
  predict_boxcox_model <- function(model, newdata) {
    pred_transformed <- predict(model, newdata)
    
    # Inverse Box-Cox transformation
    if (abs(model$lambda) < 1e-6) {
      pred_original <- exp(pred_transformed)
    } else if (abs(model$lambda - 1) < 1e-6) {
      pred_original <- pred_transformed  # No transformation case
    } else {
  ...
  # Walk-forward validation implementation (same as other models)
  ...
  diagnostics_env[[stock_name]][["LinearCov_BoxCox_AICc"]] <- list()
  
  retro_preds <- list()
  
  cat("Processing stock:", stock_name, "with Box-Cox+AICc\n")
  ...
  for (age_val in all_ages) {
    ...
    cat("  Processing Age", age_val, "with Box-Cox+AICc and covariates:", paste(valid_covars, collapse = ", "), "\n")
    ...
    for (validation_yr in retro_yrs_age) {
      ...
      tryCatch({
        ...
        pred_val <- predict_boxcox_model(fit, validation_data)
        ...
        cat("    Error fitting Box-Cox+AICc for Age", age_val, "Year", validation_yr, ":", e$message, "\n")
      })
    }
  }
  ...
  if (length(error_metrics_by_age) > 0) {
    ...
    arr_new <- array(mat, dim = c(1, nrow(mat), ncol(mat)),
                     dimnames = list("LinearCov_BoxCox_AICc", metrics, fixed_ages))
    ...
    cat("Successfully added LinearCov_BoxCox_AICc metrics for", stock_name, "\n\n")
  }
  ...
  for(age_val in all_ages) {
    ...
    tryCatch({
      # Use Box-Cox transformation and AICc selection
      y_vals <- train_data$Average_Terminal_Run
      ...
      # Generate all possible models for AICc selection
      all_models <- list()
      for (i in 1:length(valid_covars)) {
        for (combo in combn(valid_covars, i, simplify = FALSE)) {
          formula_str <- paste("Average_Terminal_Run ~", paste(combo, collapse = " + "))
          tryCatch({
            model <- lm(as.formula(formula_str), data = train_data_transformed)
            if (length(model$coefficients) <= nrow(train_data_transformed)) {
              aicc_value <- AICc(model)
              all_models[[length(all_models) + 1]] <- list(model = model, aicc = aicc_value, formula = formula_str)
            }
      ...
      if (length(all_models) > 0) {
        best_model_info <- all_models[[which.min(sapply(all_models, function(x) x$aicc))]]
        fit <- best_model_info$model
  ...
  if(length(preds_by_age) > 0) {
    ...
    row <- c(Model = "LinearCov_BoxCox_AICc", preds_by_age, Total = total_pred)
    ...
    cat("Added LinearCov_BoxCox_AICc forecasts to table.ptf for", stock_name, "\n")
  }
